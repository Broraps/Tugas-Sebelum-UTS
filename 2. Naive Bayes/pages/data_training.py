...
if "data_training" not in st.session_state:
    st.session_state.data_training = pd.read_csv("pinjaman.csv")

st.title("🧾 Kelola Data Training")
...
with st.form("input_form"):
    ...
    with col2:
        ...
        Lulus_Kredit = st.selectbox("Lulus Kredit", ["Tidak Layak", "Layak"])
    ...
    if submit:
        new_row = {
            ...
            "Lulus_Kredit": Lulus_Kredit
        }
        st.session_state.data_training = pd.concat(
            [st.session_state.data_training, pd.DataFrame([new_row])],
            ignore_index=True
        )
...
if not st.session_state.data_training.empty:
    row_to_delete = st.selectbox(
        "Pilih baris yang ingin dihapus (berdasarkan index):",
        st.session_state.data_training.index.tolist()
    )

    if st.button("HAPUS BARIS TERPILIH"):
        st.session_state.data_training = st.session_state.data_training.drop(row_to_delete).reset_index(drop=True)
